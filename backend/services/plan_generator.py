    ...
    async def generate_plan_workouts(
        ...
        try:
            ...
            _ai_prompt = (
                ...
                "workouts, NAME each specific exercise by name (e.g. weighted box step-ups, single-leg "
                "squats, Bulgarian split squats, walking lunges, calf raises, plank variations) together "
                "with its exact sets x reps or duration and rest interval, with EACH exercise as its own "
                "separate sentence — never combine multiple exercises into one comma-separated sentence, "
                "and NEVER write a vague summary like 'Complete the muscular endurance circuit for 25 "
                "minutes' or 'Complete the strength circuit' without naming the exercises; a runner "
                "unfamiliar with gym terminology must be able to follow the session exactly from this text "
                "alone; for Interval workouts, state exact rep count, distance or duration per rep, and "
            ...
            nb_schema_block = (
                ...
                "Strength/ME MUST name each specific exercise (e.g. weighted step-ups, split squats, walking "
                "lunges, calf raises) with exact sets/reps/rest per exercise, ONE exercise per sentence — "
                "never a vague 'complete the circuit' summary with no named exercises; Intervals state exact "
                "reps/distance-or-duration/recovery; plus Overall/Reason/Benefit/Warning), "
                "fueling_tip (string), "
