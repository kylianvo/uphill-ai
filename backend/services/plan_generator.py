    ...
    async def generate_plan_workouts(
        ...
        try:
            ...
            _ai_prompt = (
                ...
                "Process (step-by-step execution using → to separate segments; the warm-up, main, and "
                "cool-down minutes stated MUST sum exactly to duration_minutes. For Easy/Tempo/Interval/"
                "Long Run, e.g. 'Warm up 10 min easy → 4 x 6min @ Zone 4, 2min jog recovery → cool down 10 "
                "min'. For Strength/Muscular Endurance workouts, the MAIN portion of this SAME → chain — "
                "between the warm-up and cool-down segments — MUST itself be built from named exercises: "
                "one → segment per exercise, each stating the exact exercise name plus its sets x reps (or "
                "duration) and rest interval, e.g. 'Warm up 5 min gentle mobility → Bodyweight Squats: "
                "2x10, 60s rest → Walking Lunges: 2x10 each leg, 60s rest → Glute Bridges: 2x12, 60s rest "
                "→ Cool down 5 min stretching'. NEVER substitute a placeholder segment like 'Perform the "
                "bodyweight strength circuit for 20 minutes' for the named-exercise segments, and NEVER "
                "place the exercise breakdown anywhere outside this Process → chain (in particular, never "
                "append it after Warning or any other section) — every exercise MUST live inside Process "
                "and nowhere else. For Interval workouts, state exact rep count, distance or duration per "
                "rep, and recovery between reps), "
            ...
            nb_schema_block = (
                ...
                "for Strength/ME the MAIN segments of this SAME → chain (between warm-up and cool-down) "
                "MUST be named exercises with exact sets/reps/rest, ONE exercise per → segment, e.g. 'Warm "
                "up 5min → Squats: 2x10, 60s rest → Lunges: 2x10/leg, 60s rest → cool down 5min' — never a "
                "placeholder like 'perform the circuit for 20 min', and NEVER place exercises outside "
                "Process (e.g. appended after Warning, which is injury risks ONLY, never exercise "
                "prescriptions); Intervals state exact reps/distance-or-duration/recovery; plus "
                "Overall/Reason/Benefit/Warning), "
                "fueling_tip (string), "
