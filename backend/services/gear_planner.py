...
MATCHING: weigh each catalog entry's own fields against the athlete's criteria — "foot_shape" against the requested Width, "carbon_plate" against the Carbon Plate preference, "terrain" and "cushioning" against the requested Trail Terrain/Cushioning, and "suitability"/"highlights" against the athlete profile and special requirements (e.g. injury history, heavier runners). Prefer entries whose fields explicitly match over entries where the detail is missing.
