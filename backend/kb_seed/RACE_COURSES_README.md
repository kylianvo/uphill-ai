...
1. **Add `"race_courses"` to `DOMAINS`** in `services/kb_distiller.py` and register it with `db.replace_kb_chunks` / `load_seed` (no sweep needed for this domain — it's hand-curated, so you'd add a `load_seed("race_courses")` call at startup/import time rather than a `_distill_race_courses` sweep function).
