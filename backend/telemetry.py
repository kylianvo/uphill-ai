from prometheus_client import Histogram, Counter

notebooklm_latency_seconds = Histogram(
    'notebooklm_latency_seconds',
    'Time spent waiting for NotebookLM API response (excluding cache hits)',
    ['service']
)

notebooklm_tokens_sent_total = Counter(
    'notebooklm_tokens_sent_total',
    'Total estimated tokens sent to NotebookLM API',
    ['service']
)

notebooklm_tokens_received_total = Counter(
    'notebooklm_tokens_received_total',
    'Total estimated tokens received from NotebookLM API',
    ['service']
)
