...
class GearMetadata(BaseModel):
    brand: str = Field(description="The brand of the shoe (e.g., Hoka, Salomon, Kailas, Asics, Adidas, Nike, Puma, New Balance, On, Mount to Coast, Norda, NNormal)")
    ...
    terrain: list[str] = Field(description="List of suitable terrains (e.g., 'rocky', 'muddy', 'runnable', 'technical', 'road', 'trail')")
    ...
    community_score: float = Field(description="Overall rating or community score out of 100 (or convert out of 10). Extract as a number out of 100 if possible, or fallback to 0.0.")
    ...
    carbon_plate: bool = Field(description="True if the shoe contains a carbon plate, carbon rods, or similar stiffening element.")
    summary: str = Field(description="A concise 3-sentence summary of the shoe, its best use cases, and overall feel (e.g., plush, responsive).")
