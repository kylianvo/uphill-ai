...
- `NOTEBOOKLM_NOTEBOOK_ID`, `NOTEBOOKLM_NUTRITION_ID`, `NOTEBOOKLM_AUTH_JSON` — the NotebookLM notebooks (scheduler, nutrition-principles) used by the offline KB distiller and the legacy runtime engine. `NOTEBOOKLM_GEAR_ID` is unused now that gear sources from web discovery instead — left defined for now, harmless
